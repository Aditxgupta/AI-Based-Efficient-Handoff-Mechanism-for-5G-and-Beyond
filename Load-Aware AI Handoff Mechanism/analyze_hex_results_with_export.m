...
% =========================================================================
% PHASE 2 ANALYSIS: Metrics, Multi-Plot Visualization & Data Export
% 1. Runs deterministic inference simulation.
% 2. Generates comprehensive plots (Trajectory & Time Series).
% 3. Exports time-series data to 'simulation_log.csv'.
% =========================================================================
...
history.handoff_event = zeros(maxSteps, 1); % 1 if handoff happened, 0 otherwise
history.call_dropped = false(maxSteps, 1);

obs = reset(env);
total_reward = 0;
prev_serving_cell = -1; % To detect handoff
...
for i = 1:maxSteps
    ...
    history.ue_pos_x(i) = loggedData.UE_Position(1);
    history.ue_pos_y(i) = loggedData.UE_Position(2);
    ...
    if i > 1 && (history.serving_cell(i) ~= history.serving_cell(i-1))
        history.handoff_event(i) = 1;
    else
        history.handoff_event(i) = 0;
    end
    ...
    for k = 1:7
        ...
        % Note: Using the Environment's Tx Power logic (Assuming 30dBm as per previous discussion)
        % If you changed Tx Power in environment, update this '30' to match.
        history.all_rsrps(i, k) = 30 - pl; 
    end
...
history.ue_pos_x = history.ue_pos_x(1:actualSteps);
history.ue_pos_y = history.ue_pos_y(1:actualSteps);
...
history.handoff_event = history.handoff_event(1:actualSteps);
...
% Ping-Pong Detection
T_pp = 2.0; % Time window in seconds
steps_pp = T_pp / params.time_step;
num_pingpongs = 0;
handoff_indices = find(history.handoff_event == 1);

for k = 1:length(handoff_indices)-1
    idx_1 = handoff_indices(k);
    idx_2 = handoff_indices(k+1);
    
    if (idx_2 - idx_1) <= steps_pp
        cell_A = history.serving_cell(idx_1 - 1); 
        cell_C = history.serving_cell(idx_2); 
...
% Optionally add neighbor RSRPs if needed
% for k = 1:7
%     data_table.(['RSRP_gNB' num2str(k)]) = history.all_rsrps(:,k);
% end
...
if ~isempty(handoff_indices)
    ho_x = history.ue_pos_x(handoff_indices);
    ho_y = history.ue_pos_y(handoff_indices);
...
disp('Analysis Complete. CSV file saved.');